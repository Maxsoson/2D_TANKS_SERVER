...
export default class Game {
  ...
  onVictory() {
    ...
    let stars = 1;
    if (timeSpent <= 37000) stars = 2;
    if (timeSpent <= 34000) stars = 3;

    // ✅ Відправка результатів на сервер
    const userId = window.userId || localStorage.getItem("user_id");

    console.log("🎯 Sending progress for level 2...", {
      user_id: userId,
      score: score,
      stars: stars,
      level: this.stageIndex + 2  // 👈 Для рівня 2
    });
